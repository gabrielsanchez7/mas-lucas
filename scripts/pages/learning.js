...
const tabPosition = () => {
  ...
  const moveIndicator = () => {
    const tab = document.querySelector('#learning .training--active')
    const bounds = tab.getBoundingClientRect()
    const container = document.querySelector('.container')
    const boundsContainer = container.getBoundingClientRect()
    const indicator = document.querySelector('#learning .indicator__arrow')
    indicator.style.left = `${bounds.left - boundsContainer.left + (bounds.width / 2) - 15}px`
  }

  moveIndicator()

  const tabs = document.querySelectorAll('#learning .training__action')
  tabs.forEach(it => {
    it.addEventListener('click', e => {
      const t = e.target
      const parent = t.closest('.training')
      const sibs = Functions.allSiblings(parent)
      
      parent.classList.add('training--active')
      sibs.forEach(sib => sib.classList.remove('training--active'))
      moveIndicator()

      const step = t.getAttribute('data-training')
      const currentContent = document.querySelector('.content--active')
      const content = document.querySelector(`.content[data-training="${step}"]`)
